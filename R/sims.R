...
# Save the plot
ggsave("time_series_plot.png", width = 8, height = 6, dpi = 300)
...
population_dynamics <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    # ODEs for population dynamics
    dN1 <- r1 * N1 * (1 - (N1 + alpha12 * N2 + alpha13 * N3) / K1)
    dN2 <- r2 * N2 * (1 - (N2 + alpha21 * N1 + alpha23 * N3) / K2)
    dN3 <- r3 * N3 * (1 - (N3 + alpha31 * N1 + alpha32 * N2) / K3)
    list(c(dN1, dN2, dN3))
  })
}
...
ggplot(out_long, aes(x = time, y = Population, color = Species)) +
  geom_line(size = 1) +
  labs(title = "Population Dynamics of Three Interacting Species",
       x = "Time",
       y = "Population Size",
       color = "Species") +
