...
#| label: fig-case2
#| echo: false
#| fig-cap: "NOTE: A placeholder figure for now. It's not correct. A simple adult stage Dynamic Energy Budget (DEB) model for the grasshopper showing how food gets assimilated into reserve energy stores. The grasshopper than aloocates some fraction of the energy in reserves to maintaining body stucrture (somatic maintenance) and growth (i.e., structure) and to developing reproductive tissue (reproductive maintenance) and allocating to reproduction."
...
#| fig-cap: "Life-cycle of a grasshopper (Species A) at a location near Renmark, South Australia. Life-cycle of the grasshopper is shown in under two scenarios, one where stomata are open all day, and one where stomatal closure is driven by photosynthetically active radiation intensity and vapour pressure."
#| code-fold: true
#| echo: true
#| warning: false
#| message: false
...
#' @param time Time step
#' @return     Next years population size
ricker_two <- function(N0_i, r_i, sd_r_i, K_i, N0_j, alpha_ij, time){
  ...
  for(j in 1:nsims){
    for(i in 2:time){
      ...
      # Calculate the population size in the next time step for species i
        N_array_i[i,j] <- ricker_two(N_array_i[i-1, j], r_i, sd_r_i, K_i, N_array_j[i-1, j], alpha_ij, time)

      # Calculate the population size in the next time step for species j
        N_array_j[i,j] <- ricker_two(N_array_j[i-1, j], r_j, sd_r_j, K_j, N_array_i[i-1, j], alpha_ij, time)
    }
  }
...
geom_ribbon(aes(ymin = N_m - sd, ymax = N_m + sd), alpha = 0.3, colour = NA) +
  ...
  scale_colour_viridis_d() + scale_fill_viridis_d() + theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14)) +   annotation_custom(closed, xmin = 1, xmax = 4, ymin = 7500, ymax = 12000) + ylim(0, 12000) + scale_colour_viridis_d(
...
## **Case Study 4: Simulating the effect of watering on plant and insect body temperatures** {#case4}
...
## **Case Study 5: Impacts of watering scenerios on grasshopper life cycle** {#case5}
...
### Use the different watering scenerios to look at how changes in temperature during the watering regime period affect the grasshopper life cycle 
