...
# Create hourly values, but water during at midnight for one hour (00:00-01:00) (midnight is used to avoid changes too rapid for the integrator to solve).
rainhour_10 <- unlist(lapply(RAINFALL2, function(x) c(rep(0,23), x)))
rainhour_20 <- unlist(lapply(RAINFALL3, function(x) c(rep(0,23), x)))
...
Now we have the new microclimate objects. The original, the 10mm and the 20mm watering events to calculate body and plant leaf temperature to compare how much this impacts body temperatures experienced.
...
# Extract the microclimate temperatures. Because this was simulated for a plant the 'body tempertature' is the leaf temperature and is stored as 'TC' in the environ object

    # Load plant microclimate object with orginal rainfall
        load(here("output", "microclim", "plant_micro_micro.RDa"))
...
# Extract the same data but for the other two watering scenarios
  # Load plant microclimate object with 10mm of water
        load(here("output", "microclim", "plant_rain_10_shade_micro.RDa"))
    # Extract and store leaf temps
            environ_plant_10 <- as.data.frame(plant$environ)
  # Load plant microclimate object with 20mm of water
        load(here("output", "microclim", "plant_rain_20_shade_micro.RDa"))
    # Extract and store leaf temps
