...
We can also re-parameterise to use a finite rate model given the grasshoppers are annual species.
...
For the stomata-closed scenario ($\lambda$ = `r round(exp(0.395539), 2)`), the population could tolerate up to `r round((1-(1/exp(0.395539)))*100, 2)`% mortality per generation and still remain stable.

For the stomata-open scenario ($\lambda$ = `r round(exp(-1.33451),2)`), the population is declining rapidly. Even with no mortality, it would require a `r abs(round((1-(1/exp(-1.33451)))*100, 2))`% increase in survival or reproduction to be viable, suggesting the population cannot persist under current conditions.
...
### Impacts of watering scenerios on grasshopper life cycle
Now that we have the leaf temperature of the plant under the various watering scenerios, we can use this to understand how the different scenerios might affect grasshopper body temperature, life cycle and population dynamics like we did in [case study 2](#case2). This is a nice demonstrations of re-applying everything we've learnt to a new problem. Again, we can do this by using the ectotherm model using the leaf temperature under the different watering scenerios as the microclimate for the grasshopper. Given the short period of thea heat wave and watering regimes we don't expect major differences but it's better to check.
