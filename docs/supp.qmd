...
# Thermal Load sensitivity parameters for species A
 ctmax <- 50           # CTmax1h
     z <- -4.5          # z (slope)
...
kdot_ref <- 0.05       # repair rate % per minute at ref temp 25 deg C
      tb <- environ_A_alive$TC # body temps, Celsius
    time <- environ_A_alive$time_min # time in minutes
...
damage <- damage_accum(time = rep(60, length(tb)), 
                       temp = tb, 
                  intercept = alpha, 
                      slope = beta)
...
debout_post_hatch_A <- update_p_surv(debout_post_hatch_A, 
                                    ...
                                    m_tls = damage_w_repair)    # Heat load mortality
...
debout_post_hatch_B <- get_alive(ectoB)
    ...
    environ_B_alive <- get_environ_alive(debout_post_hatch_B, environ_B)
   

```


```{r, fig-grasshopperspA_lifecycle_box2}
