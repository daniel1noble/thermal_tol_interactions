...
pacman::p_load(devtools, deSolve, ggplot2, dplyr, tidyr, purrr, patchwork, equatags, flextable, NicheMapR, terra, microclima, plantecophys, magick, here, ggrepel, lutz, R.matlab)
...
We can use a simple Ricker model, which we'll use to simulate the population dynamics of the grasshopper. The Ricker model is a simple model that describes how populations grow over time. The model is given by the equation:
...
r_closed <- -1.33451 # Intrinsic growth (DEB model) with stoma closed
  r_open <- 0.395539 # Intrinsic growth (DEB model) with stoma closed
...
p1 = ggplot(sims_closed, aes(x = time, y = N, colour = sim)) +
  geom_line() +
  labs(title = "Ricker model simulation (Stoma Close)", x = "Generation", y = "Population size (N)") + theme_classic() + theme(legend.position = "none") +
...
p2 = ggplot(sims_open, aes(x = time, y = N, colour = sim)) +
  geom_line() +
  labs(title = "Ricker model simulation (Stoma Open)", x = "Generation", y = "Population size (N)") + theme_classic() + theme(legend.position = "none") +
  scale_colour_viridis_d() + theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14))
