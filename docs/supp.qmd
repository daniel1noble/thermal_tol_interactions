...
Our next case study will explore how the microclimate of a plant leaf can influence the life-cycle of an insect. In this case, we will use a dynamic energy budget model to understand how the microclimate of a plant leaf can influence the growth and development of an insect.
...
WETAIR_out <- WETAIR(db = micro$metout[,"TALOC"], rh = micro$metout[,"RHLOC"], bp = P_a)
    VPD_Pa <- WETAIR_out$esat - WETAIR_out$e
 photo_out <- Photosyn(Tleaf = micro$environ[,"TC"],                 # Leaf temperature (C)
                          g1 = 5.2,                                  # Parameter of Ball-Berry model
                          Ca = 400,                                  # Atmospheric CO2 concentration (ppm)
                         VPD = VPD_Pa/1000,                          # Vapour pressure deficit (kPa)
                      vpdmin = 0.5,                                  # lower threshold on VPD
