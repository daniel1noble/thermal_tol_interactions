...
# Subset debout to period when alive
...
# limit envirn data to period when alive, use TB for TLS
...
# Takes the debout post hatch data and updates survival probabilities based on additional mortality rates including activity based, hatch mortality, and thermal load stress mortality
...
#' @title get_lifecycle_vitals
#' @description Function to extract key life cycle details from DEB model output (survival, fecunity, lifespan) 
#' alive_debout=debout_post_hatch_A, ecto=ectoA
...
# Thermal Load sensitivity parameters for species A. Note that CTmax and z are values derived from a regression between temperture (yaxis) and log time to 50% mortality (xaxis). See Arnold et al. 2025 for more details. We need to invert these values to get the intercept and slope for the damage accumulation function.
 ctmaxA <- 50            # CTmax1h
     zA <- -1.0          # z (slope), negative indicates decrease in survival with time
