...
pacman::p_load(devtools, deSolve, ggplot2, dplyr, tidyr, purrr, patchwork, equatags, flextable, NicheMapR, terra, microclima, plantecophys, magick, here, ggrepel, lutz, R.matlab, magick, grid, DT)
...
Modelling the dynamics between two species requires some knowldge about how competition impacts the population size of each species. Normally, this is achives by estimating competition coefficients form field or laboratory data. Experiments involve determining how the presence of one species impacts growth of the other relative to a control state where only the single species is present. Given this is a simple case study to demonstarte how to link teh various models, we'll assume that the competition coefficients are known and are as follows:
