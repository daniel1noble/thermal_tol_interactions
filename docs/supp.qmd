...
We can see from @fig-grasshopper_lifecycle that the life-cyle of the grasshopper is different when we consider the behaviour of the plant stomata. Notably, when stomata are closed and the leaf temperatures are higher we expect the grasshopper to move through stages faster and reproduce more (@tbl-grasshopper_lifecycle). We can have a look at the different traits expected under both scenarios. Looking at the body temperstures we see major spikes after the life cyle is complete. This is because the grasshopper has reached the end of its lifespan, so the mictoclimate predcitions after this are not relevant.
...
Recall from our previous section that we already estimated the intrinsic growth rate. Using this information, for the stomata-closed scenario ($\lambda$ = `r round(exp(0.395539), 2)`), the population could tolerate up to `r round((1-(1/exp(0.395539)))*100, 2)`% mortality per generation and still remain stable.

In contrast, for the stomata-open scenario ($\lambda$ = `r round(exp(-1.33451),2)`), the population is declining rapidly. Even with no mortality, it would require a `r abs(round((1-(1/exp(-1.33451)))*100, 2))`% increase in survival or reproduction to be viable, suggesting the population cannot persist under current conditions.
