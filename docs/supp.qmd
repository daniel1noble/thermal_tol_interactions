...
# Traits we want to extract from the DEB model
 traits <- c("FECUNDITY", "CLUTCHES", "LifeSpan", "R0") 
...
r_closed <- 1.503123 # Intrinsic growth (DEB model) with stoma closed
  r_open <- 0.069320 # Intrinsic growth (DEB model) with stoma closed
...
N_closed <- sim_ricker(N0, r_closed, sd_r, K, time, nreps) 
  N_open <- sim_ricker(N0,   r_open, sd_r, K, time, nreps)
...
sims_closed <- pivot_longer(as.data.frame(N_closed), 
                        ...
                        names_to = "sim", 
                       values_to = "N")  %>% 
               mutate(time = rep(1:time, each = nreps))
