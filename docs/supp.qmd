...
Now, for 40% shade we can use the argument `rainhourly = 1` to create hourly values and then feedl in what the rainfall. 
...
rainhour_10 <- unlist(lapply(RAINFALL2, function(x) c(x, rep(0,23))))

rainhour_20 <- unlist(lapply(RAINFALL3, function(x) c(x, rep(0,23))))
...
# Now, run the plant biophysical model using each of the three different microclimate objects.

objects <- c("microclimate.RDa", "water_10_shade.RDa", "water_20_shade.RDa")
  names <- c("real", "water_10", "water_20")
...
We've now used the heat budget model aplying each of the three different miroclimate objects to calculate the leaf temperature of the plant. We can now plot this out to see how the watering events affect leaf temperature.
...
# Now lets plot the leaf temperature of the plant over time for each of the three different watering scenarios
  environ_plant2 <- environ_plant2 %>% filter(dates > "2019-01-15" & dates < "2019-01-31") %>%
              dplyr::select(dates, TC, df_name) %>%
              pivot_longer(cols = c(TC), 
                           names_to = "Temps", 
                           values_to = "Temperature") %>% 
              mutate(Temps = ifelse(Temps == "TC",  "Leaf Temperature (40% shade)"))

  ggplot(environ_plant2) + 
              geom_line(aes(y = Temperature, x = dates, col = df_name), show.legend = TRUE) +
              labs(y = "Temperature (°C)", x = "Date") + theme_classic() +
              theme( axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), legend.position = "top") + 
              scale_colour_viridis_d()
```
