...
ricker <- function(N0, r, sd_r, K, time){
  N0 * exp(log(rnorm(1, r, sd_r))) * (1 - (N0/K))
}
...
r_closed <- 1.503123 # Intrinsic growth (DEB model) with stoma closed
  r_open <- 0.069320 # Intrinsic growth (DEB model) with stoma closed
    sd_r <- 0.25     # Standard deviation of the growth rate
...
sims_closed <- pivot_longer(as.data.frame(N_closed), 
                      cols = everything(), 
                      names_to = "sim", 
                      values_to = "N")  %>% mutate(time = rep(1:time, each = nreps))
...
sims_open <- pivot_longer(as.data.frame(N_open), 
                      ...
                      values_to = "N")  %>% mutate(time = rep(1:time, each = nreps))
# Plot the results
ggplot(sims, aes(x = time, y = N, colour = sim)) +
  geom_line() +
  labs(title = "Ricker model simulation", x = "Time", y = "Population size (N)") + theme_classic() + theme(legend.position = "none") +
  scale_colour_viridis_d()
