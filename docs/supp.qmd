...
damage_repair_accum <- function(time, temp, intercept, slope, threshold = 0.5, TA, TAL, TAH, TL, TH, TREF, kdot_ref) {
  ...
  for(i in 1:length(temp)){
    ...
    damage_repair[i] <- damage$damage_i[i] - (repair_rate * (time[i+1] - time[i]))  # time in minutes
  }
  
  # If repair outweighs damage and the combined values are less than 0 then assign them zero.
  damage_repair[damage_repair < 0] <- 0

  output$damage_repair <- damage_repair[!is.na(damage_repair)]

  # Step 3: Sum the increments cumulatively to get accumulated heat damage through time

  output$cum_damage_repair[1] <- output$damage_repair[1]
  for (j in 2:nrow(output)) {
    output$cum_damage_repair[j] <- output$cum_damage_repair[j - 1] + output$damage_repair[j]
  }

  # Convert HI to survival probability at each time step given a threshold value used (50%, or 80% usually)
  output$mortality_prob = (output$cum_damage_repair * threshold) / 100

  # Now check that mortality cum damage and probabilities do not exceed 1 (100%)
          output$mortality_prob[output$mortality_prob > 1] <- 1
  output$cum_damage_repair[output$cum_damage_repair > 100] <- 100
...
# Thermal Load sensitivity parameters for species A. Note that CTmax and z are values derived from a regression between temperture (yaxis) and log time to 50% mortality (xaxis). See Arnold et al. 2025 for more details. We need to invert these values to get the intercept and slope for the damage accumulation function.
 ctmaxA <- 50            # CTmax1h
     zA <- -0.19          # z (slope), negative indicates decrease in survival with time which it must do as animals don't get better the longer they are at stressful temps
