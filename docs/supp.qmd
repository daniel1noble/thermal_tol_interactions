...
#| code-fold: true
  ...
  environ_plant_plantecophys <- environ_plant_plantecophys2 %>% filter(dates > "2019-01-15" & dates < "2019-01-18") %>%
              ...
              pivot_longer(cols = c(TC), 
                           names_to = "Temps", 
                           values_to = "Temperature") %>% 
              mutate(Temps = ifelse(Temps == "TC",  "Leaf Temperature (Stomatal Closed) (40% shade)"))
  ...
  ggplot(new_environ_plant2) + 
              ...
              theme( axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),legend.position = "right", legend.text = element_text(size = 14), axis.text = element_text(size = 14), axis.title = element_blank()) + 
...
# Invoke movement behaviour such that, once a certain temperature is reached by the plant the grasshopper will seek lower parts of the plant where stomata are open. Simulating theormoregulatioon in space within the plant. 
     behaviour <- TRUE    # Set to TRUE to invoke behaviour, FALSE to not
threshold_temp <- 43      # degrees C for animal to move into cooler parts of the plant with stomata open

# plot(plant2$environ[,"TC"], type = "l")
if(behaviour){
  # Check where plant temperature exceeds threshold temp. If so, then assume grasshopper moves into lower parts of plants and microclimate becomes that of plant2 with different stomatal behaviour.
  plant2$environ[which(plant2$environ[,"TC"] > threshold_temp),] <- plant$environ[which(plant2$environ[,"TC"] > threshold_temp),]
} 

## Check that it worked
# lines(plant2$environ[,"TC"], col = "red") # Clearly as thresholds reached the new microclimate environment experieced by the grasshopper is that of the plant with stomata open because it migrates down into the plant where it's cooler.
...
load(here("output", "microclim", "grasshopperspA_stomata_open_Box1.RDa"))
load(here("output", "microclim", "grasshopperB_stomata_closed_Box1.RDa"))
...
# Merge the two dataframes
  temp_combined <- rbind(
    temp_spA_open %>% mutate(Species = "Species A (Stomata Open)"),
    temp_spB_open %>% mutate(Species = "Species B (Stomata Closed)")
  )
...
ggplot(temp_combined_sub, aes(x = datetime, y = TA, color = Species)) +
  ...
  scale_colour_manual(values = c("Species A (Stomata Open)" = "#356920", "Species B (Stomata Closed)" = "#664229"))
...
load(here("output", "microclim", "grasshopperspA_stomata_open_Box1.RDa")) #ectoA
load(here("output", "microclim", "grasshopperB_stomata_closed_Box1.RDa"))     #ectoB
...
# Thermal Load sensitivity parameters for species A. Note that CTmax and z are values derived from a regression between temperture (yaxis) and log time to 50% mortality (xaxis). See Arnold et al. 2025 for more details. We need to invert these values to get the intercept and slope for the damage accumulation function.
 ctmaxB <- 47              # CTmax1h
     zB <- -2            # z (slope), negative indicates decrease in survival with time
