...
#' )
damage_repair_accum <- function(x, TA, TAL, TAH, TL, TH, TREF, kdot_ref) {
  ## ---- Basic checks ---------------------------------------------------------
  if (!is.numeric(x)) stop("'x' must be numeric (°C).")
  ...
  TK   <- 273.15 + x     # evaluation temperatures (K)
  # Scalars below are already in Kelvin per argument definitions:
  # TL, TH, TREF

  ## ---- Numerator: Arrhenius rise relative to TREF ---------------------------
  # exp( TA * (1/TREF - 1/T) )
  num <- exp(TA * (1 / TREF - 1 / TK))

  ## ---- Denominator: low & high inactivation terms ---------------------------
  # 1 + exp( TAL * (1/T - 1/TL) ) + exp( TAH * (1/TH - 1/T) )
  den <- 1 + exp(TAL * (1 / TK - 1 / TL)) + exp(TAH * (1 / TH - 1 / TK))

  ## ---- Combine with reference rate ------------------------------------------
  out <- (num / den) * kdot_ref
...
# Lets use air temperature as a test case for functions, limit to 2m above ground (TA). Note that this dataset has dates but we need to convert these to cumulated time in minutes for the damage and repair functions to work. We have hourly data so we simply multiply the row number by 60 to get time in minutes.

air_2m_temp <- air_temp2  %>% filter(Height == "2 m")  %>% mutate(time_min = c(0:(n()-1))*60)


# Test from Arnold et al. 2025a parameters
ctmax <- 52 # CTmax1h
z <-  2.5 # z (slope)
...
ta <- air_2m_temp$Temperature # time vector, minutes

# No repair
damage <- damage_accum(time = air_2m_temp$time_min, 
             temp = air_2m_temp$Temperature, 
         intercept = alpha, 
              slope = beta)
par(mfrow = c(1, 2))
plot(air_2m_temp$time_min, damage, type = "l", xlab = "Time (min)", ylab = "Accumulated Damage (%)", main = "No Repair", xlim = c(0, 30000))
plot(air_2m_temp$time_min, air_2m_temp$Temperature, type = "l", xlab = "Time (min)", ylab = "Air Temperature (°C)", main = "Air Temperature at 2m", xlim = c(0, 30000))
```
